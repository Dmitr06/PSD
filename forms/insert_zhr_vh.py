...
def insert_zhr_vh(akt_vh, road_programm, road_db, tube, km_start, km_finish, dy_tube):
    ...
    # -----------------------Материалы
    num_start = 39  # начинаем заполнять с данной строки
    col_stand = ['P', 'R', 'X', 'AL']
    '''for i in range(39, 41):
        ws_l1['A' + str(i)] = akt_vh[0]['date'] + ' г.'
        ws_l1['AF' + str(i)] = '%s %s\n_________________\n%s' % (
        akt_vh[0]['otv'][1], akt_vh[0]['otv'][2], akt_vh[0]['otv'][0])
        ws_l1['AI' + str(i)] = '%s %s\n_________________\n%s' % (
        akt_vh[0]['contr'][1], akt_vh[0]['contr'][2], akt_vh[0]['contr'][0])'''
    # ------Создаем поля согласно бд
    for _, x in enumerate(akt_vh):
        ...
        for y in range(1, 46):
            ws_l1.cell(num_start, y)._style = ws_l2.cell(1, y)._style
        index_mat = 1
    ...
    if not os.path.exists(road_db):
        os.makedirs(road_db)
    road_db = os.path.join(road_db, 'Журнал входного контроля.xlsx')
    wb_zhr.save(road_db)
