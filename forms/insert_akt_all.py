...
def insert_akt_all(def_db,road_programm,road_db,tube,km_start,km_finish,dy_tube):
    ...
    for i,x in enumerate(def_db):
        ...
        for rows_tabl in  range(26,25+len(x['defect'])):
            ...
            for coord in ('B{0}:D{0}','E{0}:G{0}','H{0}:J{0}','K{0}:O{0}','P{0}:Q{0}','R{0}:S{0}','T{0}:U{0}','T{0}:U{0}','V{0}:X{0}','Y{0}:AA{0}'):
        ...
        ws5=wb['Укладка тп']
        ws5['A7']=x['otv'][2]
        ws5['S7']='Устранение дефектов на секциях %s, %s-%s км, Ду %s мм.'%(tube,km_start,km_finish,dy_tube)
        ws5['AD12']=x['km']+' км.'
        ws5['AB19']=x['date'][2].strftime('%d.%m.%Y')+' г.'
        ws5['K24']='Дист. %s м/%s км'%(x['dist'][3],x['km']) 
        ws5['V24']='Дист. %s м/%s км'%(x['dist'][4],x['km']) 
        ws5['M27']=str(round(x['dist'][4]-x['dist'][3],1))+' м.п.'
        ws5['I46']=' '.join((x['contr'][1],x['contr'][2],x['contr'][0]))    # поменять первую букву регистра с большой на маленькую
        ws5['I49']=' '.join((x['otv'][1],x['otv'][2],x['otv'][0]))
        ws5['I52']=' '.join((x['sk'][1],x['sk'][2],x['sk'][0]))
        ws5['A21']=ws5['A21'].value % (ws5['I52'].value,ws5['I46'].value,ws5['I49'].value,ws5['M27'].value)
        #--------------Акт объемов
        ws6=wb['Объемы']
        ws6['O9']=i+1
        ...
        for cell_table in ('B25','B27','B28','B32'):
