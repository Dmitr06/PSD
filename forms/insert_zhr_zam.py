...
def insert_zhr_zam(def_db,road_programm,road_db,tube,km_start,km_finish,dy_tube):
    ...
    ws['D4'] = def_db[0]['otv'][2]
    ws['A15'] = 'Устранение дефектов на секциях %s, %s-%s км, Ду %s мм.'%(tube,km_start,km_finish,dy_tube)
    ws['AL25'] = def_db[0]['otv'][2]
    ws['AL27'] = def_db[0]['date'][0].strftime('%d.%m.%Y')+ ' г.'
    ws['AL29'] = def_db[-1]['date'][2].strftime('%d.%m.%Y')+ ' г.'
    #-----------------------Заключение
    temp=def_db[-1]
    ws['AR71']='%s %s %s _____________ %s'%(temp['otv'][1],temp['otv'][2],temp['otv'][0],ws['AL29'].value)
    road=road_db.rpartition('/')[0]+'/Журналы'
    if not os.path.exists(road):
        os.makedirs(road)    
    wb_zhr.save(road+'/2. Журнал замечаний.xlsx')
