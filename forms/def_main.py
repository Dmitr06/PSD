...
def def_form(db_from_start, all_otv, all_control, all_sk, all_lkk):  # основное окно
    def add_s(def_sec):  # Добавление дефектной секции (в db_from_start)
        ...
        for i in range(len(db_from_start)):
            db_from_start[i]['rand_value'][0] = random.uniform(0.25,
                                                               0.6)  # Добавление рандомных велечин для котлована и изоляции
            db_from_start[i]['rand_value'][1] = random.uniform(2.5,
                                                               3.8)  # Добавление рандомных велечин для котлована и изоляции
        ...
        def_ent_sec.delete(0,
                           END)  # очищает строку ввода номера секции после введения(+)
    ...
    def update_lbox(a):  # обновление информации в frame03 по щелчку в listbox
        ...
        if db_from_start[temp]['date'][
            0] != None:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
        ...
        if 'type_base' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
            var_type_base.set(db_from_start[temp]['type_base'])
        if 'otv' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
            var_otv.set(db_from_start[temp]['otv'][0])
        if 'contr' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
            var_contr.set(db_from_start[temp]['contr'][0])
        if 'sk' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
            var_sk.set(db_from_start[temp]['sk'][0])
        if 'lkk' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
            var_lkk.set(db_from_start[temp]['lkk'][0])
        if 'grnd_maker' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
            var_grnd_maker.set(db_from_start[temp]['grnd_maker'][0])
        if 'grnd_contr' in db_from_start[
            temp]:  # Если новая запись, то остается значени предыдщей записи, если есть - присваивается
    ...
    def save_s():  # сохранение во временной БД (db_from_start)
        ...
        if temp_dist != '':
            ...
            try:
                ...
                db_from_start[temp]['dist'][1] = round(
                    db_from_start[temp]['dist'][0] - db_from_start[temp]['rand_value'][0] - result,
                    2)  # начало изоляиии
                db_from_start[temp]['dist'][2] = round(
                    db_from_start[temp]['dist'][0] + db_from_start[temp]['rand_value'][0] + result, 2)  # конец изоляции
                db_from_start[temp]['dist'][3] = round(
                    db_from_start[temp]['dist'][1] - db_from_start[temp]['rand_value'][1] - result,
                    2)  # начало котлована
                db_from_start[temp]['dist'][4] = round(
                    db_from_start[temp]['dist'][2] + db_from_start[temp]['rand_value'][1] + result,
                    2)  # конец котлована
            except ValueError:
                showerror('Не сохранено!', 'Правильный формат:12345.64')
        try:
            temp_dl_muft = (def_ent_dl_muft.get()).replace(',', '.')
            db_from_start[temp]['dl_muft'] = float(temp_dl_muft)  # запись длины устанавливаемой муфты
        except ValueError:
            showerror('Не сохранено!', 'Правильный формат:1.5')
...
if __name__ == '__main__':
    ...
    all_control = {
        '': ('', '', '')}  # для выпадающего списка с контролирующими лицами + пустая запись(если отсутствует)
    all_sk = {'': ('', '', '')}  # для выпадающего списка с строительным контролем + пустая запись(если отсутствует)
    all_lkk = {'': ('', '', '')}
    for (i, j) in zip(('otv.txt', 'control.txt', 'sk.txt', 'lkk.txt'),
                      (all_otv, all_control, all_sk, all_lkk)):  # Чтение ответственных из файлов с людьми
        try:
