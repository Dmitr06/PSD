...
def insert_main(def_db,road_programm,road_db,tube,km_start,km_finish,dy_tube):
    ...
    for i in def_db:
        ...
        ws1['AF38'] = ws1['AF38'].value + '\n' + i['zakl']
        ws1['AF39'] = ws1['AF39'].value + '\n' + i['zakl'].replace('В', 'У')
        ws1['AF40'] = ws1['AF40'].value + '\n' + i['zakl'].replace('В', 'К')
    ...
    for defect in def_db:
        for (x,y,z) in zip((defect['contr'],defect['lkk'],defect['sk'],defect['otv']),(contr,lkk,sk,otv),('A2','A3','A4','A5')):
            if (x not in y) and (x[0] != ''):
                y.append(x)
                if (y is contr) or (y is otv): height_row = 135
                else: height_row = 60
    ...
    index_row += 1
    temp_row = str(index_row)
    if 'ЦРС "Рязань"' in def_db[0]['otv'][2]:
        ws2['BC' + temp_row] = 'Начальник ЦРС "Рязань" Борисов А.А.'
    elif 'ЛПДС "Рязань"' in def_db[0]['otv'][2]:
        ws2['BC' + temp_row] = 'Зам. начальника ЛПДС "Рязань" Костин В.К.'
    else:
        ws2['BC' + temp_row] = 'Начальник ППС "Плавск" Красильников А.В.'
...
if __name__=='__main__':
    ...
    def_db=[{'sec': '12',
            ...
            {'sec' : '13',
             ...
             'defect': {'1': {'sec': '13', 'dist': '12313', 'lab': 'Потеря метала', 'dl': '12', 'sh': '132', 'gl': '123', 'type': 'Шлифовка'}, '2': {'sec': '13', 'dist': '2344234', 'lab': 'Потеря метала', 'dl': '34', 'sh': '4343', 'gl': '434', 'type': 'Шлифовка'}}, 'otv': ('Кулешов А.Б.', 'Начальник РУ№3', 'ЦРС "Рязань"'), 'contr': ('Клименко Д.А.', 'Начальник ЛАЭС №1', 'ППС "Плавск'), 'sk': ('', '', ''), 'lkk': ('Козин А.П.', 'Инженер-дефектоскопист', 'ЛККиД')},
            {'sec': '14',
             ...
             'defect': {'1': {'sec': '14', 'dist': '1131213,32', 'lab': 'Риска', 'dl': '1233', 'sh': '12331', 'gl': '2133', 'type': 'Муфта П1'}, '2': {'sec': '14', 'dist': '321313,21', 'lab': 'Риска', 'dl': '23', 'sh': '234', 'gl': '2342', 'type': 'Муфта П1'}}, 'otv': ('Макаров М.А.', 'Начальник АРС', 'ЛПДС "Рязань"'), 'contr': ('Клименко Д.А.', 'Начальник ЛАЭС №1', 'ППС "Плавск'), 'sk': ('Мирошкин М.В.', 'Инженер СК', 'ООО "Сег'), 'lkk': ('Козин А.П.', 'Инженер-дефектоскопист', 'ЛККиД')},
            {'sec': '15',
             'date': [date(2018, 12, 14),
                      date(2018, 12, 14),
                      date(2018, 12, 15)],
             ...
             'defect': {'1': {'sec': '15', 'dist': '123131,12', 'lab': 'Риска', 'dl': '123', 'sh': '123', 'gl': '13', 'type': 'Муфта П2'}, '2': {'sec': '15', 'dist': '1231', 'lab': 'Риска', 'dl': '3213', 'sh': '123', 'gl': '312', 'type': 'Муфта П2'}, '3': {'sec': '15', 'dist': '432423', 'lab': 'Риска', 'dl': '13', 'sh': '123', 'gl': '13', 'type': 'Муфта П2'}}, 'otv': ('Кулешов А.Б.', 'Начальник РУ№3', 'ЦРС "Рязань"'), 'contr': ('Клименко Д.А.', 'Начальник ЛАЭС №1', 'ППС "Плавск'), 'sk': ('sdfsdf', '3213123', 'fsd'), 'lkk': ('Козин А.П.', 'Инженер-дефектоскопист', 'ЛККиД')}]
