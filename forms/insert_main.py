...
def create_row (ws, ws2, current_row, coord, height=13.50):
    ...
    ws.merge_cells('D{0}:N{0}'.format(current_row))
    ws.merge_cells('O{0}:BG{0}'.format(current_row))
...
def insert_main(def_db,road_programm,road_db,tube,km_start,km_finish,dy_tube):
    ...
    index_row += 11
    ws4['A'+str(index_row)]=' '.join((def_db[-1]['sk'][1],def_db[-1]['sk'][2],def_db[-1]['sk'][0]))
    ws4.print_area = 'A1:AV'+str(index_row+16)    
    #--------------Акт передачи
    ws5=wb['Akt']
    index_row = 16
    for section in def_db:
