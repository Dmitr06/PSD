...
def save_db():  # Фукнция сохранить БД
    global road_db
    main_theme = 'Устранение дефектов методом выборчного ремонта на секциях %s, %s-%s км, Ду %s мм.' % tuple(
        data_tube[i].get() for i in data)
    if road_db == '':
        save_as_db()
    else:
        try:
            if road_db[-4:-3] == '.':
                road_db = road_db[
                          :road_db.rfind('.')]  # если перезаписываем файл, то файл идет с расширением, убираем его.
...
def save_akt_vh():  # функция скнопки сохранения полей акта
    try:
        ...
        for (i, j) in zip(('date', 'doc', 'kol'), (date_vh, doc_vh, kol_vh)):  # записываем поля ввода
            akt_vh[temp][i] = j.get()
        for (i, j, n) in zip(('otv', 'contr', 'kk', 'sk'), (var_otv, var_contr, var_kk, var_sk),
                             (all_otv, all_control, all_control, all_sk)):
...
for (i, j) in zip(('otv.txt', 'control.txt', 'sk.txt', 'lkk.txt', 'materials.txt'),
                  (all_otv, all_control, all_sk, all_lkk, materials)):  # Чтение ответственных из файлов с людьми
    try:
...
export_menu.add_command(label='1. Входной контроль', command=lambda: insert_vh(akt_vh, road_programm, road_db,
                                                                               tuple(data_tube[i].get() for i in data)))
export_menu.add_command(label='2. Журналы', command=lambda: insert_zhr(def_db, akt_vh, road_programm, road_db,
                                                                       tuple(data_tube[i].get() for i in data)))
export_menu.add_command(label='3. Акты', command=lambda: insert_akts(def_db, road_programm, road_db,
                                                                     tuple(data_tube[i].get() for i in data)))
...
Button(frame_2, text='Тыц', command=lambda: def_main.def_form(def_db, all_otv, all_control, all_sk, all_lkk)).pack(
    fill=X)
